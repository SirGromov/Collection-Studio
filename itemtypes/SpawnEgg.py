...
class SpawnEgg:
    ...
    def __init__(self, name, ench, lore, atts, rarity, itype, amount, initial, meta):
        ...
        self.nbt = '{display:{Name:"' + self.name + '",Lore:[' + self.lore_array + ']},ench:' + self.ench_list_nbt + \
                   ',AttributeModifiers:['+self.attribute_nbt_list+']'+',HideFlags:'+str(self.hidden)+','+self.mobtag+'}'
    ...
    def mob_parser(self):
        res = {'health': '0', 'attack': '0', 'id': ''}
        for i in self.meta:
            lst = i.split(': ')
            if lst[0] == 'монстр': res['id'] = mobs[lst[1]]
            if lst[0] == 'хп': res['health'] = lst[1]
            if lst[0] == 'атака': res['attack'] = lst[1]
        self.mobtag = 'EntityTag:{id:"minecraft:'+res['id']+'",Health:'+res['health']+',Attributes:[{Name:"generic.maxHealth",Base:'+\
                      res['health']+'},{Name:"generic.attackDamage",Base:'+res['attack']+'}]}'
